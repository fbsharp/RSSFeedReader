...
---
title: "App features for RSS Feed Reader (Node.js + Express)"
description: "Stakeholder user-facing feature requirements for the RSS Feed Reader MVP, with developer notes adapted for the Node.js + Express stack"
author: "Paula Silva"
date: "2026-06-10"
version: "1.0.0"
status: "approved"
...
- Use simple in-memory storage (a JavaScript array in the Express server)
- No need for feed parsing libraries yet
- No HTTP client or fetching logic needed for MVP
- Focus on basic UI and API communication (the frontend calls the API with the browser `fetch` API using relative URLs)
**For Extended-MVP (add feed fetching):**
- Use the `rss-parser` npm package for fetching and parsing (`parser.parseURL(feedUrl)` handles both in a single call)
